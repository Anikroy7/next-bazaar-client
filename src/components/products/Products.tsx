...
function Products() {
    ...
    return (
        <div>
            ...
            {
                (data?.data && data?.data.length ? data?.data?.map((product:TProduct) => <ProductCard key={product.id} product={product} />) : <p className='text-center text-xl text-gray-600'>No produts found!!</p>)
            }
        </div>
    )
}
