...
import { useDeleteCupon, useGetAllCupons, useUpdateCupon } from '@/src/hooks/cupon.hook'
...
export default function AllCupons() {
    ...
    const { mutate: updateCupon, isPending: updateCuponPending } = useUpdateCupon();
    ...
    return (
        <div>
            <div className="flex justify-between items-center mt-4 py-8 px-4">
                ...
                <Button onClick={()=>router.push(`/dashboard/vendor/manage-cupons/add`)} className="flex items-center space-x-2 bg-default-900 text-default">
                ...
                <TableBody>
                    {data?.data?.map((cupon: TCupon) =>
                        <TableRow key={cupon.id}>
                            ...
                            <TableCell>{new Date(cupon.createdAt).toLocaleString()}</TableCell>
                            <TableCell>
                                <CuponProductsModal key={cupon.id} products={cupon?.appliedProducts} />
                            </TableCell>
                            <TableCell>
                                <div className="relative flex items-center gap-2">
                                    ...
                                    {cupon.isActive ? (
                                        <Button
                                            ...
                                            size="sm"
                                            variant="solid"
                                            onClick={() => handleUpdateCuponStatus("DEACTIVE", cupon.id)}
                                        >
                                            {(id === cupon?.id && updateCuponPending) ? "Loading..." : "Deactive"}
                                        </Button>
                                    ) : (
                                        <Button
                                            className='mr-5'
                                            ...
                                            size="sm"
                                            variant="solid"
                                            onClick={() => handleUpdateCuponStatus("ACTIVE", cupon.id)}
                                        >
                                            {(id === cupon?.id && updateCuponPending) ? "Loading..." : "Active"}

                                        </Button>
                                    )}
                                    <Tooltip color="danger" content="Delete cupon">
                                        <span className="text-lg text-danger cursor-pointer active:opacity-50">
                                            <DeleteIcon onClick={() => handleDeleteCupon(cupon.id)} />
                                        </span>
                                    </Tooltip>
